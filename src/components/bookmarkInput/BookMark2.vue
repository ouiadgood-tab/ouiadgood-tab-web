<template>
    <div>
      <div class="item-container" @mouseover="showTimesIcon">
        ...
        <TheBookMark2 v-if="showBookmark" @close-bookmark="hideBookmark"/>
    </div>
</template>

<script>
import TheBookMark2 from './TheBookMark2.vue';
...
export default {
  ...
  computed: {
    bookmarks() {
      const storedBookmarks = localStorage.getItem("bookmarks2");
      if (storedBookmarks) {
  ...
  methods: {
    ...
    deleteBookmark(){
      localStorage.removeItem("bookmarks2");
      window.location.reload(); // Reload the page
    },
  },
  components: { TheBookMark2 },
};
</script>
