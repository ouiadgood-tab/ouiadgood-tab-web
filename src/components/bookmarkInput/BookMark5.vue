<template>
  <div>
    <div class="item-container" @mouseover="showTimesIcon" >
      ...
      <TheBookMark5 v-if="showBookmark" @close-bookmark="hideBookmark"/>
  </div>
</template>

<script>
import TheBookMark5 from './TheBookMark5.vue';
...
computed: {
  bookmarks() {
    const storedBookmarks = localStorage.getItem("bookmarks5");
    if (storedBookmarks) {
...
methods: {
  ...
  deleteBookmark(){
    localStorage.removeItem("bookmarks5");
    window.location.reload(); // Reload the page
  },
},
components: { TheBookMark5 },
};
</script>
