<template>
  <div>
    <div class="item-container" @mouseover="showTimesIcon">
      ...
      <TheBookMark4 v-if="showBookmark" @close-bookmark="hideBookmark"/>
  </div>
</template>

<script>
import TheBookMark4 from './TheBookMark4.vue';
...
computed: {
  bookmarks() {
    const storedBookmarks = localStorage.getItem("bookmarks4");
    if (storedBookmarks) {
...
methods: {
  ...
  deleteBookmark(){
    localStorage.removeItem("bookmarks4");
    window.location.reload(); // Reload the page
  },
},
components: { TheBookMark4 },
};
</script>
